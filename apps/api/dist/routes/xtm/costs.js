...
const xtmCostRoutes = async (fastify) => {
    // POST /:projectId/costs — generate cost record; returns {costId} only
    fastify.post(`${BASE}/costs`, async (request, reply) => {
        const projectId = parseInt(request.params.projectId, 10);
        const db = fastify.mongo;
        const project = await core_1.Collections.projects(db).findOne({ projectId });
        if (!project)
            return reply.status(404).send({ error: 'Project not found' });
        const jobs = await core_1.Collections.jobs(db).find({ projectId, status: 'FINISHED' }).toArray();
        ...
        if (project.freelancerId) {
            const fl = await core_1.Collections.freelancers(db).findOne({ freelancerId: project.freelancerId });
            if (fl) {
        ...
        await core_1.Collections.costs(db).insertOne(cost);
    ...
    fastify.post(`${BASE}/costs/:costId/pos`, async (request, reply) => {
        const projectId = parseInt(request.params.projectId, 10);
        const costId = parseInt(request.params.costId, 10);
        const db = fastify.mongo;
        const cost = await core_1.Collections.costs(db).findOne({ costId, projectId });
        if (!cost)
        ...
        await core_1.Collections.purchaseOrders(db).insertOne(po);
    ...
    fastify.get(`${BASE}/costs/:costId/pos`, async (request, reply) => {
        const projectId = parseInt(request.params.projectId, 10);
        const costId = parseInt(request.params.costId, 10);
        const db = fastify.mongo;
        const [cost, po] = await Promise.all([
            core_1.Collections.costs(db).findOne({ costId, projectId }),
            core_1.Collections.purchaseOrders(db).findOne({ costId, projectId }),
        ]);
        if (!cost || !po)
            return reply.status(404).send({ error: 'PO not found' });
        let vendorFirstName = 'LLM';
        let vendorLastName = '';
        if (po.freelancerId) {
            const fl = await core_1.Collections.freelancers(db).findOne({ freelancerId: po.freelancerId });
            if (fl) {
                const [first, ...rest] = fl.name.split('-');
    ...
    fastify.get(`${BASE}/custom-fields`, async (request, reply) => {
        const projectId = parseInt(request.params.projectId, 10);
        const project = await core_1.Collections.projects(fastify.mongo).findOne({ projectId });
        if (!project)
            return reply.status(404).send({ error: 'Project not found' });
        return reply.send([
    ...
    fastify.post(`${BASE}/custom-fields`, async (request, reply) => {
        const projectId = parseInt(request.params.projectId, 10);
        const db = fastify.mongo;
        ...
        if (Array.isArray(fields)) {
            ...
            if (ceId?.value?.value !== undefined) {
                await core_1.Collections.projects(db).updateOne({ projectId }, { $set: { referenceId: ceId.value.value, updatedAt: new Date() } });
            }
        }
        return reply.send({ success: true });
    });
};
exports.default = xtmCostRoutes;
