 ...
 */
const trainingDataRoutes = async (fastify) => {
    /**
    ...
    fastify.get('/admin/api/training-data', async (request, reply) => {
        ...
        if (isNaN(since.getTime()))
            return reply.status(400).send({ error: `Invalid 'since' date: ${q.since}` });
        if (isNaN(until.getTime()))
            return reply.status(400).send({ error: `Invalid 'until' date: ${q.until}` });
        ...
        const segments = await core_1.Collections.segments(db)
            .find(segmentFilter)
            .sort({ updatedAt: 1 })
            .limit(limit)
            .toArray();
        ...
        const records = segments
            ...
            if (q.sourceLang && project?.sourceLanguage?.toUpperCase() !== q.sourceLang.toUpperCase())
                return null;
            if (q.targetLang && project?.targetLanguage?.toLowerCase() !== q.targetLang.toLowerCase())
                return null;
            return {
    ...
    fastify.get('/admin/api/training-data/stats', async (_request, reply) => {
        const db = fastify.mongo;
        const agg = await core_1.Collections.segments(db)
            .aggregate([
...
exports.default = trainingDataRoutes;
