...
const adminProjectRoutes = async (fastify) => {
    ...
    fastify.get('/admin/api/projects', async (request, reply) => {
        ...
        if (q.customerId)
            filter['customerId'] = parseInt(q.customerId, 10);
        const limit = Math.min(parseInt(q.limit ?? '50', 10), 200);
        const skip = parseInt(q.skip ?? '0', 10);
        const db = fastify.mongo;
        const [projects, total] = await Promise.all([
            core_1.Collections.projects(db).find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit).toArray(),
            core_1.Collections.projects(db).countDocuments(filter),
        ]);
        ...
        const [customers, jobCounts] = await Promise.all([
            core_1.Collections.customers(db).find({ customerId: { $in: customerIds } }).toArray(),
            Promise.all(projects.map((p) => core_1.Collections.jobs(db).countDocuments({ projectId: p.projectId }).then((c) => ({
    ...
    fastify.get('/admin/api/projects/:projectId', async (request, reply) => {
        const projectId = parseInt(request.params.projectId, 10);
        const db = fastify.mongo;
        const [proj, jobs] = await Promise.all([
            core_1.Collections.projects(db).findOne({ projectId }),
            core_1.Collections.jobs(db).find({ projectId }).sort({ createdAt: 1 }).toArray(),
        ]);
        if (!proj)
            return reply.status(404).send({ error: 'Project not found' });
        const customer = await core_1.Collections.customers(db).findOne({ customerId: proj.customerId });
        return reply.send({
    ...
    fastify.get('/admin/api/stats', async (_request, reply) => {
        const db = fastify.mongo;
        const [totalProjects, statusCounts, recentProjects, recentCallbacks, segmentCounts, wordAgg,] = await Promise.all([
            core_1.Collections.projects(db).countDocuments(),
            core_1.Collections.projects(db)
                .aggregate([{ $group: { _id: '$status', count: { $sum: 1 } } }])
                .toArray(),
            core_1.Collections.projects(db).find().sort({ createdAt: -1 }).limit(5).toArray(),
            core_1.Collections.callbackLogs(db).find().sort({ createdAt: -1 }).limit(6).toArray(),
            core_1.Collections.segments(db)
                .aggregate([{ $group: { _id: '$matchType', count: { $sum: 1 } } }])
                .toArray(),
            core_1.Collections.jobs(db)
                .aggregate([
        ...
        const customers = await core_1.Collections.customers(db)
    ...
    fastify.post('/admin/api/projects', async (request, reply) => {
        ...
        const template = await core_1.Collections.templates(db).findOne({
            method: body.method,
...
async function fireAdminCallback(db, broker, opts) {
    if (!opts.url)
        return;
    const now = new Date();
